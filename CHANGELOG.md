...
- Added streaming support to `LlmComposer.Agent`: when `settings.stream_response` is `true`, `LlmComposer.Agent.run/3` returns `{:ok, stream}` where `stream` is a lazy `Enumerable` of `LlmComposer.StreamChunk`. The loop runs intermediate tool-calling turns internally (reassembling streamed tool-call deltas into complete calls via the new `LlmComposer.Agent.StreamCollector`) and forwards only the final, tool-free answer to the caller token-by-token, ending with a terminal `:done` chunk that carries cumulative `:usage`/`:cost_info` and the full `LlmComposer.Agent.Result` in `metadata.agent_result`. Supported for `:open_ai`, `:open_router`, `:google`, and `:bedrock` providers (others yield a terminal `:error` chunk).
- Extended `LlmComposer.Agent.StreamCollector` to support `:open_router` (reuses the OpenAI index-based fragment reassembly — identical wire format) and `:bedrock` (new strategy: groups `"inputJson"` deltas by `"toolUseId"` and preserves insertion order via a `tool_id_sequence` list, since Bedrock continuation chunks carry no identity).
- Extended `LlmComposer.Agent.StreamCollector` to support `:open_ai_responses` (groups `"function_call_arguments_delta"` fragments by `"call_id"`, seeded by a preceding `"function_call_started"` event) and `:ollama` (text streaming only — Ollama's native format does not emit tool-call deltas; for tool-call streaming use the `:open_ai` provider pointed at Ollama's OpenAI-compatible endpoint). The streaming agent's supported provider list is now `:open_ai`, `:open_router`, `:open_ai_responses`, `:google`, `:bedrock`, `:ollama`.
- Added `:tool_call` stream chunk type to `LlmComposer.StreamChunk`: the streaming agent now emits one assembled `:tool_call` chunk per executed tool call (with the result filled in) right after execution and before the next LLM turn. This makes the stream self-contained — callers can react to tool calls inline without needing a `:telemetry` handler.
- Enriched `LlmComposer.Agent` telemetry so UIs can drive progress over `:telemetry` (both synchronous and streaming modes): `[:llm_composer, :agent, :tool, :start]` now includes `:arguments`, `:metadata` and `:id` (not just `:name`); a new `[:llm_composer, :agent, :reasoning, :delta]` event surfaces intermediate reasoning while streaming; and a new `:telemetry_metadata` option to `run/3` is merged — together with an auto-generated `:run_id` — into the metadata of every agent event so a handler can scope itself to a single run (e.g. to broadcast via `Phoenix.PubSub`).
