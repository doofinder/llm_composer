...
- Added streaming support to `LlmComposer.Agent`: when `settings.stream_response` is `true`, `LlmComposer.Agent.run/3` returns `{:ok, stream}` where `stream` is a lazy `Enumerable` of `LlmComposer.StreamChunk`. The loop runs intermediate tool-calling turns internally (reassembling streamed tool-call deltas into complete calls via the new `LlmComposer.Agent.StreamCollector`) and forwards only the final, tool-free answer to the caller token-by-token, ending with a terminal `:done` chunk that carries cumulative `:usage`/`:cost_info` and the full `LlmComposer.Agent.Result` in `metadata.agent_result`. Supported for the `:open_ai` and `:google` providers (others yield a terminal `:error` chunk).
